...
export default async function ExamStartPage() {
  ...
  if (!session?.user?.id) {
    redirect('/signin?callbackUrl=/exam/start') // CHANGED: exam-specific auth callback path.
  }

  const quizzes = await getStartableQuizzes()

  const examQuizzes = quizzes.filter((quiz) =>
    quiz.allowedModes.includes('exam'),
  ) // CHANGED: keep CPD quizzes out of exam mode.
    ...
    <main className='space-y-4 sm:space-y-6'>
      <section className='rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6'>
        ...
        <p className='mt-1 text-sm text-slate-600 dark:text-slate-400'>
          Select a published quiz and begin an exam-mode attempt.
        </p>
      </section>
